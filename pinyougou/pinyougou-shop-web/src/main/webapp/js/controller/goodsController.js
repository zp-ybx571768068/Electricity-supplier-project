app.controller("goodsController", function ($scope, $controller, $location, goodsService, uploadService,
    ...
    //商品提交审核
    $scope.submitAudit = function (status) {
        ...
        if(confirm("确定要更新选中的商品状态吗？")){
            goodsService.submitAudit($scope.selectedIds, status).success(function (response) {
                if(response.success) {
                    //刷新列表并清空选中的那些商品
                    $scope.reloadList();
                    $scope.selectedIds = [];
    ...
    //修改商品上下架
    $scope.isPutAway= function (marketable) {
        if($scope.selectedIds.length < 1){
            alert("请先选择要上架或下架的商品");
            return;
        }
        if(confirm("确定要上架或下架的已选择记录吗")){
            goodsService.isPutAway($scope.selectedIds,marketable).success(function (response) {
                if(response.success){
                    $scope.reloadList();
                    $scope.selectedIds = [];
                } else {
